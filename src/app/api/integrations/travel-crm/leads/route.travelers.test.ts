...
describe("Travelers overrides → Travel CRM payload", () => {
  it("14–18. counts and ages map to the exact payload keys", async () => {
    const res = await POST(
      post({
        ...
        overrides: {
          ...
          infants: "1",
          childrenWithBedAges: ["4", "6"],
          childrenWithoutBedAges: ["8"],
          infantAges: ["1"],
        },
      }),
    );
  ...
  it("mapped Workspace counts reach the payload unedited", async () => {
    h.flow_runs[0].vars = {
  ...
  it("dialog edits win over mapped answers without touching stored data", async () => {
    h.flow_runs[0].vars = {
      ...((h.flow_runs[0].vars ?? {}) as Record<string, unknown>),
      adults: "4",
    };
    const before = JSON.stringify(h.flow_runs[0].vars);
    const res = await POST(
      post({
        flow_id: "flow-1",
        flow_run_id: "run-1",
        overrides: { ...FUNNEL, adults: "2" },
      }),
    );
    expect(((await res.json()) as Record<string, unknown>).success).toBe(true);
    expect(h.travelCalls[0].body).toMatchObject({ adults: 2 });
    expect(JSON.stringify(h.flow_runs[0].vars)).toBe(before);
  });

  it("prefill echoes traveler counts and ages for dialog re-rounds", async () => {
    h.workspace_values = [];
    const res = await POST(
      post({
        flow_id: "flow-1",
    ...
    expect(json.prefill).toMatchObject({
      adults: 2,
      childrenWithBed: 1,
      childrenWithBedAges: [5],
...
describe("Travelers validation blocks invalid Creates", () => {
  it.each([
    ["0", "Adults must be at least 1."],
    ["", "Adults is required."],
    ["-1", "Adults must be a whole number."],
    ["two", "Adults must be a whole number."],
  ])("rejects adults=%j", async (adults, message) => {
    const res = await POST(
      post({ flow_id: "flow-1", flow_run_id: "run-1", overrides: { ...FUNNEL, adults } }),
    );
    const json = (await res.json()) as {
      success: boolean;
      code: string;
    ...
    expect(json.fields.adults).toEqual([message]);
    expect(h.travelCalls).toHaveLength(0);
  });

  it("rejects bad child counts and ages without calling Travel CRM", async () => {
    for (const overrides of [
      { childrenWithBed: "two" },
      { infants: "-1" },
      { childrenWithBedAges: ["abc"] },
      { infantAges: ["120"] },
    ]) {
      const res = await POST(
        post({ flow_id: "flow-1", flow_run_id: "run-1", overrides: { ...FUNNEL, ...overrides } }),
      );
      expect(((await res.json()) as { code: string }).code).toBe("INVALID_OVERRIDES");
    }
    expect(h.travelCalls).toHaveLength(0);
  });

  it("blank optional ages never block creation", async () => {
    const res = await POST(
      post({
        flow_id: "flow-1",
        flow_run_id: "run-1",
    ...
    expect(((await res.json()) as Record<string, unknown>).success).toBe(true);
    expect(h.travelCalls[0].body).toMatchObject({ childrenWithBedAges: [] });
  });
});
