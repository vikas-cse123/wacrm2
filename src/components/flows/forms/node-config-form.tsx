...
// ============================================================
// send_cta_url — text (+ optional image) with a button that opens a link
// ============================================================

interface SendCtaUrlCfg {
  text?: string;
  header_image_url?: string;
  footer_text?: string;
  button_text?: string;
...
function SendCtaUrlForm({
  ...
  return (
    <>
      <div>
        <label className="mb-1 block text-xs text-muted-foreground">
          Header image (optional) — shown above the text
        </label>
        {cfg.header_image_url ? (
          <div className="flex items-center gap-2 rounded-md border border-border bg-muted px-3 py-2 text-xs">
            <Paperclip className="h-3.5 w-3.5 shrink-0 text-cyan-400" />
      ...
      <div>
        <label className="mb-1 block text-xs text-muted-foreground">
          Button link (the URL it opens)
        </label>
        <Input
          value={cfg.button_url ?? ""}
          onChange={(e) => onUpdateConfig({ button_url: e.target.value })}
          placeholder="https://calendly.com/…"
          className="bg-muted text-xs"
        />
        <p className="mt-1 text-[10px] text-muted-foreground">
          Tapping the button opens this link. It doesn&apos;t reply, so the
          flow continues to the node below.
        </p>
      </div>
...
function GoogleSheetsSyncForm({
  ...
  const importPast = async (window_: { from?: string; to?: string }) => {
    setImporting(true);
    try {
              ...
              <div className="mt-3 rounded-md bg-muted/40 p-2.5">
                ...
                <div className="mt-2">
                  <ImportDateRangePopover
                    triggerLabel="Import past completed responses"
                    triggerClassName="h-7 text-xs"
                    onConfirm={importPast}
                    busy={importing}
                    disabled={busy}
                  />
                </div>
