-- ============================================================
-- 080_dashboard_daily_contacts.sql — single contacts KPI + daily chart
--
-- Problem: the dashboard showed four KPI cards (Total Messages, Unique
-- Contacts, New Contacts, New Conversations) but the product question
-- is "how many distinct contacts did we message". The message/new
-- cards are removed; the KPIs object now carries ONLY uniqueContacts
-- (true COUNT(DISTINCT contact_id) over messages in range — never
-- messages, never double-counted).
--
-- It also adds the "Contacts Messaged — Last 30 Days" chart source:
-- `dailyContacts` — one entry per calendar day for the trailing 30
-- days in the CALLER's timezone (same tz semantics as the monthly
-- buckets), each counting DISTINCT contacts with >=1 message that
-- day. A contact with ten messages on one day counts once; silent
-- days are present with 0 (LEFT JOIN from generate_series).
--
-- What changed vs 079 (CREATE OR REPLACE — no schema change, same
-- signature, same security posture: SECURITY INVOKER, account from
-- auth.uid(), authenticated-only grant):
--   - `kpis`: only `uniqueContacts` (current + previous). The
--     totalMessages / newContacts / newConversations counts are
--     gone — the UI no longer renders those cards.
--   - New CTEs (daily_bounds -> daily_msg -> dailyContacts key).
--     Same messaging semantics as every other metric (messages
--     scoped through their conversation's account, NULL contacts
--     ignored).
--   - Flow breakdown (per-contact attribution, No flow row,
--     contacts pct) and monthly keys are byte-identical to 079.
--
