-- ============================================================
-- 079_dashboard_flow_contacts.sql — flow breakdown counts CONTACTS
--
-- Problem: the "Message Breakdown by Flow" section counted MESSAGES
-- per flow (SUM of message rows), so a single chatty contact could
-- dominate a flow's number. The dashboard should answer "how many
-- distinct contacts did each flow message in this period".
--
-- What changed vs 075 (CREATE OR REPLACE — no schema change, same
-- signature, same security posture: SECURITY INVOKER, account from
-- auth.uid(), authenticated-only grant):
--   - Range flow attribution is now per CONTACT, not per
--     conversation: each distinct contact messaged in range is
--     attributed to exactly ONE flow with the SAME pick rule as
--     before (conversation-exact run first — newest active, else
--     newest among the contact's in-range conversations — else the
--     contact's newest active run, else newest run). The rule is
--     unchanged; only the counting grain moved from messages to
--     contacts, mirroring the monthly per-(month, contact) logic.
--   - `flowBreakdown.rows`: { flowId (nullable), flowName,
--     contacts, pct }. `messages` is gone; `uniqueContacts` is
--     renamed to `contacts`.
--   - `flowBreakdown.totalContacts`: distinct contacts messaged in
--     range (the pct denominator). Rows always sum to it, so pct
--     values sum to 100 and no contact is double-counted.
--   - Unattributed contacts are now a "No flow" row (NULL flow_id,
--     name 'No flow') instead of being silently excluded — same
--     treatment as the monthly chart.
--   - Everything else (KPIs, monthlyUniqueContacts, monthlyFlows,
--     monthly CTEs, indexes) is byte-identical to 075.
--
