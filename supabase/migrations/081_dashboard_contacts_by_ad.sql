-- ============================================================
-- 081_dashboard_contacts_by_ad.sql — "Contacts by Ad" donut source
--
-- Problem: the dashboard shows how flows perform, but travel
-- businesses spend on Click-to-WhatsApp ads and need to see which
-- ad brought each contact in. WACRM already persists first-touch
-- CTWA attribution on contacts (migrations 035/036: source_url,
-- source_type, ctwa_clid — stamped once by the webhook, never
-- overwritten), so no new table or webhook change is needed.
--
-- What changed vs 080 (CREATE OR REPLACE — no schema change, same
-- signature, same security posture: SECURITY INVOKER, account from
-- auth.uid(), authenticated-only grant):
--   - New `contactsByAd` key: per-ad UNIQUE contacts messaged in
--     the selected range.
--
-- Attribution rule (documented here and mirrored in tests):
--   - Grain is the CONTACT, using its stored first-touch
--     `contacts.source_url`. `ctwa_clid` is a per-click id and is
--     NEVER used as a bucket — one contact with 20 messages from
--     the same ad counts once.
--   - NULL or blank source_url → the '__no_ad__' bucket.
--   - The top 8 ad URLs by contacts keep their own bucket
--     (ranked by contacts desc, URL asc for ties); every other
--     URL merges into '__other__'. Each contact lands in exactly
--     one bucket, so percentages sum to 100 and bucket counts sum
--     to the total. "Other" is the exact remainder, not an
--     estimate.
--   - `pct` denominator is ALL distinct contacts messaged in
--     range (=== the uniqueContacts KPI), so the donut center
--     reads "Contacts" with that total.
--   - Rows order by contacts desc with '__no_ad__' forced last.
--   - Everything else (KPIs, flow breakdown, daily, monthly
--     keys and CTEs) is byte-identical to 080.
--
...
BEGIN
  ...
  -- Distinct (day, contact) pairs in the window: one contact with
  -- ten messages on a day counts once for that day.
  daily_msg AS (
    SELECT DISTINCT
  ...
  SELECT jsonb_build_object(
    ...
    'dailyContacts', (
      ...
      FROM (
        SELECT jsonb_build_object(
          'date', gs.day::TEXT,
          'contacts', COUNT(dm.contact_id)::INT
        ) AS d
        FROM (
